...
if(isset($_POST["btnupdateorder"])){
  ...
  //write update uery for tbl_product stock.
    ...
    //write update query for tbl_invoice table data
     ...
     $update_invoice=$pdo->prepare("update tbl_invoice set customer_name=:cust,order_date=:orderdate,total=:total,paid=:paid,due=:due,payment_type=:ptype where invoice_id=$id ");
...
$row_product=$select->fetch(PDO::FETCH_ASSOC);
                       ...
                       <?php
                           ...
                           echo '<td><input type="text" class="form-control productname" name="productname[]" value="'.$row_product['pname'].'" readonly></td>';
...
<script>
    ...
    //   alert(id);
        ...
        var html='';
        html+='<tr>';  
        html+='<td><select style="width:200px;"  class="form-control productid" name="productid[]"><?php echo fill_product($pdo,$pid);  ?> </select></td>';
